...
class Mpantau extends CI_Model
{
	...
	function get_dasboard($temp)
	{
		...
		if ($temp == 3) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  1 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 4) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  2 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 5) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  3 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 6) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  4 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 7) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  5 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 8) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  6 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 9) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 10) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW() AT TIME ZONE 'Asia/Jakarta'+ INTERVAL '-1 day', 'mm-dd-yyyy' )  ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 11) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  403 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 12) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  7373 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 13) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  8172 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		} elseif ($temp == 14) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  9519 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		}elseif ($temp == 15) {
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh24:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE  ps.IdItem =  84729 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-dd-yyyy') ORDER BY ps.tanggaljam ASC";
		}
	...
	function get_jumlah()
	{
		...
		if (isset($_POST['search'])) {
			$bulanan = $this->input->post('month');

			if ($bulanan !== null) {
				$query = "SELECT kh.username AS username, ip.item AS item, kh.idkaryawan as idkaryawan, COUNT(*) FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem JOIN kh_karyawan kh ON ps.idkaryawan = kh.idkaryawan WHERE  ps.iditem =  1  AND 
				to_char(ps.tanggaljam, 'yyyy-mm') = '$bulanan' AND kh.idkaryawan = $idkaryawan
				 GROUP BY kh.username , ip.item, kh.idkaryawan";
			} else {
				$query = "SELECT kh.username AS username, ip.item AS item, kh.idkaryawan as idkaryawan, COUNT(*) FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem JOIN kh_karyawan kh ON ps.idkaryawan = kh.idkaryawan WHERE  ps.iditem =  1  AND to_char(ps.tanggaljam, 'mm-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-yyyy') AND kh.idkaryawan = $idkaryawan GROUP BY kh.username , ip.item, kh.idkaryawan";
			}
		} else {
			$query = "SELECT kh.username AS username, ip.item AS item, kh.idkaryawan as idkaryawan, COUNT(*) FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem JOIN kh_karyawan kh ON ps.idkaryawan = kh.idkaryawan WHERE  ps.iditem =  1  AND to_char(ps.tanggaljam, 'mm-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-yyyy') AND kh.idkaryawan = $idkaryawan GROUP BY kh.username , ip.item, kh.idkaryawan";
		}
	...
	function get_bulankaryawan()
	{
		...
		if (isset($_POST['search'])) {
			...
			if ($bulanan !== null) {
				$query = "SELECT kh.username AS username, ip.item AS item, kh.idkaryawan as idkaryawan , count(ps.iditem),to_char(ps.tanggaljam, 'dd-mm-yyyy') AS jam
				FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem JOIN kh_karyawan kh ON ps.idkaryawan = kh.idkaryawan 
				WHERE  ps.iditem =  1  AND 
				to_char(ps.tanggaljam, 'yyyy-mm') = '$bulanan' AND kh.idkaryawan = $idkaryawan
				GROUP BY kh.username , ip.item, kh.idkaryawan, jam";
			} else {
				$query = "SELECT kh.username AS username, ip.item AS item, kh.idkaryawan as idkaryawan, count(ps.iditem) ,to_char(ps.tanggaljam, 'dd-mm-yyyy') AS jam
				 FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem JOIN kh_karyawan kh ON ps.idkaryawan = kh.idkaryawan 
				 WHERE  ps.iditem =  1  AND 
				 to_char(ps.tanggaljam, 'mm-yyyy') = to_char(NOW()AT TIME ZONE 'Asia/Jakarta', 'mm-yyyy') AND kh.idkaryawan = $idkaryawan
				 GROUP BY kh.username , ip.item, kh.idkaryawan, jam";
			}
		} else {
			$query = "SELECT kh.username AS username, ip.item AS item, kh.idkaryawan as idkaryawan, count(ps.iditem) ,to_char(ps.tanggaljam, 'dd-mm-yyyy') AS jam
