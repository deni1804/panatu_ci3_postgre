...
class Mpantau extends CI_Model
{
	...
	function get_pantau()
	{
		...
		$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus
	...
	function get_history($temp)
	{
		if ($temp == 1) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status, ps.keterangan FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . "AND (ps.tanggaljam::date = current_date-1)  ORDER BY ps.tanggaljam DESC
			";
		} elseif ($temp == 3) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . " AND ps.IdItem =  1 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW(), 'mm-dd-yyyy') ORDER BY ps.tanggaljam DESC";
		} elseif ($temp == 4) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . " AND ps.IdItem =  2 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW(), 'mm-dd-yyyy') ORDER BY ps.tanggaljam DESC";
		} elseif ($temp == 5) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . " AND ps.IdItem =  3 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW(), 'mm-dd-yyyy') ORDER BY ps.tanggaljam DESC";
		} elseif ($temp == 6) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . " AND ps.IdItem =  4 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW(), 'mm-dd-yyyy') ORDER BY ps.tanggaljam DESC";
		} elseif ($temp == 7) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . " AND ps.IdItem =  5 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW(), 'mm-dd-yyyy') ORDER BY ps.tanggaljam DESC";
		} elseif ($temp == 8) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan,ps.tanggaljam FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . " AND ps.IdItem =  6 AND to_char(ps.tanggaljam, 'mm-dd-yyyy') = to_char(NOW(), 'mm-dd-yyyy') ORDER BY ps.tanggaljam DESC";
		} elseif ($temp == 9) {
			...
			$query = "SELECT ip.item AS item, to_char(ps.tanggaljam, 'hh:mi:ss') AS jam, ps.status,ps.tingkatstatus, ps.keterangan FROM ps_pantauansistem ps INNER JOIN ps_itempantauan ip ON ps.iditem = ip.iditem WHERE ps.idkaryawan = " . $this->session->userdata('userid') . "AND (ps.tanggaljam::date = current_date-1)  ORDER BY ps.tanggaljam DESC
			";
		}
